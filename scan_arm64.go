...
// getVLC is a branchless optimized version for ARM64.
// This version reduces branches in the hot path using arithmetic instead of conditional jumps.
// ARM64 has conditional execution and efficient bit manipulation instructions that the compiler can leverage.
func (d *decoder) getVLC(vlc *[65536]vlcCode, vlc8 *[256]vlcCode8, code *uint8) int {
	...
	if d.bufBits >= 16 && !d.markerHit {
		...
		if huffBits > 0 {
			...
			if valBits == 0 {
				...
				// ARM64 compiler will likely use CSEL (conditional select) for this
				if code != nil {
					*code = huffCode
				}
				return 0
			...
			if d.bufBits >= totalBits {
				...
				//   }
				//
				// Branchless version for ARM64:
				// ARM64 has efficient arithmetic right shift (ASR) that propagates sign bit
				// The compiler can also use CSEL (conditional select) or CSInc for branchless execution
				threshold := 1 << (valBits - 1)

				// Compute mask: -1 if value < threshold, 0 otherwise
				// Using arithmetic right shift to propagate sign bit
				// Note: int is 64-bit on arm64, so shift by 63
				signBit := (value - threshold) >> 63  // -1 if negative, 0 otherwise

				// Compute the correction term
				// Correction = ((-1) << valBits) + 1 when signBit = -1
				// = (-1 << valBits) - (-1) = -(2^valBits - 1)
				correction := (signBit << valBits) - signBit
				value += correction

				// Branchless conditional write
				// ARM64 will use conditional instructions (CSEL/CSINC)
				if code != nil {
					*code = huffCode
				}
				return value
	...
	// Branchless sign extension (same as fast path)
	threshold := 1 << (valBits - 1)
	signBit := (value - threshold) >> 63  // int is 64-bit on arm64
	correction := (signBit << valBits) - signBit
	value += correction
